...
def test_td_extract_funding_data_r6(mock_funding_sheet, mock_project_lookup):
    """Test project level funding data extracted as expected."""
...
def test_hs_extract_funding_data_r6(mock_hs_funding_sheet, mock_hs_project_lookup):
    """Test project level funding data extracted as expected."""
    extracted_funding_data = tf.extract_funding_data(mock_hs_funding_sheet, mock_hs_project_lookup, reporting_round=6)

    expected_funding_data = pd.read_csv(
        resources_assertions / "funding_data_expected_hs_r6.csv", index_col="original_index", dtype=str
    )
    # convert to datetime - datetime object serialization slightly different in csv parsing vs Excel.
    expected_funding_data["Start_Date"] = pd.to_datetime(expected_funding_data["Start_Date"])
    expected_funding_data["End_Date"] = pd.to_datetime(expected_funding_data["End_Date"])
...
def test_full_ingest_columns(mock_ingest_full_extract):
    ...
    for mapping in INGEST_MAPPINGS:
        ...
        # remove round 4 and PF specific columns
