...
from data_store.const import OUTCOME_CATEGORIES, OUTPUT_CATEGORIES
...
def test_extract_project_progress_with_float(mock_progress_sheet, mock_project_lookup):
    ...
    assert list(extracted_project_progress["delivery_rag"]) == ["3", "5.5", "2"]
    assert list(extracted_project_progress["spend_rag"]) == ["", "text", "2"]
...
def test_extract_funding_data_programme_only(mock_funding_sheet, mock_project_lookup):
    ...
    assert not extracted_funding_data["funding_source"].isin(["Town Deals 5% CDEL Pre-Payment"]).any()
...
def test_extract_funding_data_fhsf(mock_funding_sheet, mock_project_lookup):
    ...
    assert (
        not extracted_funding_data["spend_type"]
        .isin(
            [
...
def test_no_extra_projects_in_funding(mock_funding_sheet, mock_project_lookup):
    ...
    output_project_ids = set(extracted_funding_data["project_id"].unique())
...
def test_extract_outputs(mock_outputs_sheet, mock_project_lookup):
    ...
    assert set(extracted_output_ref["output_category"]) - set(OUTPUT_CATEGORIES.values()) == {"Custom"}
    # test that only outputs from outputs_data are in outputs_ref and vice-versa
    assert set(extracted_output_data["output"]) == set(extracted_output_ref["output_name"])
...
def test_extract_outcomes(mock_outcomes_sheet, mock_project_lookup, mock_programme_lookup):
    ...
    project_xor_programme = (
        extracted_outcome_data["project_id"].notnull() ^ extracted_outcome_data["programme_id"].notnull()
    )
...
def test_extract_risk(mock_risk_sheet, mock_project_lookup, mock_programme_lookup):
    ...
    assert risk_value_to_drop not in set(extracted_risk_data["short_desc"])
...
def test_full_ingest(mock_ingest_full_extract):
    ...
    valid_programmes_for_extract = {mock_ingest_full_extract["Place Details"]["programme_id"].iloc[1]}
