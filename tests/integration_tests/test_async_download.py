...
def test_invalid_file_format(test_session):
    response = test_session.get("/trigger_async_download?file_format=invalid")
...
def test_email_address_removed():
    """Test that the email_address key is removed from the query_params before calling download()"""
    with patch("core.controllers.async_download.download") as mock_download:
        mock_download.return_value = MagicMock(return_value=("application/json", io.BytesIO(b'{"data": "test"}')))
        query_params = {
            "email_address": "test_user@levellingup.gov.uk",
            "file_format": "json",
            "funds": "HS",
        }
        async_download(query_params)
        args, kwargs = mock_download.call_args
        assert "email_address" not in kwargs, "email_address key was not removed from query_params"
        assert kwargs["file_format"] == "json", "file_format was not passed to download()"
        assert kwargs["funds"] == "HS", "funds was not passed to download()"
...
def test_trigger_async_download_endpoint(mocker, seeded_test_client):
    ...
    response = seeded_test_client.get("/trigger_async_download?email_address=dev@levellingup.test&file_format=json")
