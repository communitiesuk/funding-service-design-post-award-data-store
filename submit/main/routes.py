...
@submit_blueprint.route("/login", methods=["GET"])
def login():
    return render_template("submit/login.html")


@submit_blueprint.route("/dashboard", methods=["GET"])
@login_required(return_app=SupportedApp.POST_AWARD_SUBMIT)
@set_user_access
def dashboard():
    return render_template("submit/dashboard.html", authorised_funds=g.access.items())


@submit_blueprint.route("/upload/<fund_code>/<round>", methods=["GET", "POST"])
@login_required(return_app=SupportedApp.POST_AWARD_SUBMIT)
@set_user_access
def upload(fund_code, round):
    ...
    if request.method == "POST":
        ...
        if pre_errors:
            ...
            return render_template("submit/validation-errors.html", validation_errors=validation_errors, fund=fund)
        else:
            # Success
            ...
            return render_template("submit/success.html", file_name=excel_file.filename)
