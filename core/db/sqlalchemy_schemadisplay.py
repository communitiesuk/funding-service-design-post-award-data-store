"""
This has been added to the repo to enable ERD diagrams to be generated from SQLAlchemy. It isn't installed as a pypi
library because this contains an unnoficial fix for SQLAlchemy 2.0 from
...
def _mk_label(mapper, show_operations, show_attributes, show_datatypes, show_inherited, bordersize):
    html = (
        '<<TABLE CELLSPACING="0" CELLPADDING="1" BORDER="0" CELLBORDER="%d" ALIGN="LEFT"><TR><TD><FONT POINT-SIZE="'
        '10">%s</FONT></TD></TR>' % (bordersize, mapper.class_.__name__)
    )
    ...
    if show_operations:
        html += '<TR><TD ALIGN="LEFT">%s</TD></TR>' % '<BR ALIGN="LEFT"/>'.join(
            ...
            % (
                name,
                ", ".join(
                    default is _mk_label and ("%s") % arg or ("%s=%s" % (arg, repr(default)))
...
def _render_table_html(
    ...
    schema_str = ""
    if show_schema_name == True and hasattr(table, "schema") and table.schema is not None:  # noqa
    ...
    html = (
        '<<TABLE BORDER="1" CELLBORDER="0" CELLSPACING="0"><TR><TD ALIGN="CENTER">%s%s%s</TD></TR><TR><TD BORDER="1" '
        'CELLPADDING="0"></TD></TR>' % (schema_str, "." if show_schema_name else "", table_str)
    )

    html += "".join(
        '<TR><TD ALIGN="LEFT" PORT="%s">%s</TD></TR>' % (col.name, format_col_str(col)) for col in table.columns
    )
    if session.bind and isinstance(session.bind.dialect, PGDialect):
        # postgres engine doesn't reflect indexes
        indexes = dict(
            (name, defin)
            for name, defin in session.bind.execute(
...
def create_schema_graph(
    ...
    if (
        ...
        and len(set(format_schema_name.keys()).difference({"color", "fontsize", "italics", "bold"})) > 0
    ):
    ...
    if (
        ...
        and len(set(format_table_name.keys()).difference({"color", "fontsize", "italics", "bold"})) > 0
    ):
        raise KeyError("Unrecognized keys were used in dict provided for `format_table_name` parameter")

    graph = pydot.Dot(
        prog="dot", mode="ipsep", overlap="ipsep", sep="0.01", concentrate=str(concentrate), rankdir=rankdir
    )
