...
def validate_workbook(workbook: dict[str, pd.DataFrame], schema: dict) -> list[vf.ValidationFailure]:
    ...
    constraints = (
        ...
        (validate_unique_composite_keys, "composite_keys"),
...
def validate_unique_composite_keys(
    workbook: dict[str, pd.DataFrame], sheet_name: str, composite_keys: tuple
) -> list[vf.NonUniqueCompositeKeyFailure]:
    """
    Validates the uniqueness of composite keys across multiple columns.
    ...
    :param composite_keys: A list of tuples, where each tuple contains the column names
    ...
    mask = sheet[list(composite_keys)].duplicated(keep=False)
    single_duplicated_rows = mask.drop_duplicates()
    # removing duplicates as to avoid index mismatch
    de_duplicated_sheet = workbook[sheet_name].drop_duplicates(subset=list(composite_keys))

    if single_duplicated_rows.any():
        failures = [
            vf.NonUniqueCompositeKeyFailure(sheet=sheet_name, row=tuple(row[1:]))
            for row in de_duplicated_sheet[single_duplicated_rows].itertuples(index=False)
        ]
