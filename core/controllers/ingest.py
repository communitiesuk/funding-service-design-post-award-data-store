...
def ingest(body, excel_file):
    ...
    schema = (
        modify_schema(current_app.config["VALIDATION_SCHEMA"])
...
def modify_schema(schema):
    """
    Modifies the schema by removing specified tables and adding "Submission ID" as the first column to specified tables.

    :param schema: The original schema dictionary.
    :return: The updated schema dictionary.
    """
    # Create a deep copy of the original schema to avoid modifying it directly
    updated_schema = copy.deepcopy(schema)

    # Remove "Private Investments", "Outputs_Ref", and "Output_Data" tables
    tables_to_remove = ["Private Investments", "Outputs_Ref", "Output_Data"]
    for table in tables_to_remove:
        if table in updated_schema:
            del updated_schema[table]

    # Add "Submission ID" as the first column to specified tables
    tables_to_update = [
        "Submission_Ref",
        "Programme Progress",
        "Place Details",
    ...
    new_column = {"Submission ID": "string"}
    for table in tables_to_update:
        if table in updated_schema:
            columns = updated_schema[table]["columns"]
            updated_columns = {**new_column, **columns}
            updated_schema[table]["columns"] = updated_columns

    # These fields are (largely) unpopulated for Round 1, but we want to keep rest of row
    updated_schema["Project Details"]["non-nullable"].remove("Single or Multiple Locations")
    updated_schema["Outcome_Data"]["non-nullable"].remove("Actual/Forecast")
