...
def ingest_round_1_data(data_model: dict[pd.DataFrame],
                        round_1_data: dict[pd.DataFrame]) -> dict[pd.DataFrame]:
    """
    Extract & transform data from Round 1 Reporting Template into column headed Pandas DataFrames.
    ...
    df_dictionary['Project Details'] = transform_project_location(data_model_fields['Project Details'], round_1_data['project_location'], round_1_data['lookup_programme_project'])
    df_dictionary['Project Progress'] = transform_project_progress(data_model_fields['Project Progress'], round_1_data['project_progress'], round_1_data['lookup_programme_project'])
    df_dictionary['Funding'] = transform_project_funding_profiles(data_model_fields['Funding'], round_1_data['project_funding_profiles_MASTER'], round_1_data['lookup_programme_project'])
    df_dictionary['Funding Comments'] = transform_project_funding_comments(data_model_fields['Funding Comments'], round_1_data['project_funding_comments'], round_1_data['lookup_programme_project'])

    project_risks = transform_project_risks(data_model_fields['RiskRegister'], round_1_data['project_risks'], round_1_data['lookup_programme_project'])
...
def extract_programme_risks(df_programme_risks: pd.DataFrame) -> pd.DataFrame:
    ...
    df_programme_risks = df_programme_risks[['risk_name', 'risk_category', 'short_description_of_the_risk',
                                             'full_description', 'consequences', 'pre-mitigated_impact',
                                             'pre-mitigated_likelihood', 'mitigations', 'post-mitigated_impact',
                                             'post-mitigated_likelihood', 'proximity']]
    
    # Rename columns to correspond to data model names for easier merge
    df_programme_risks = df_programme_risks.rename(columns={'risk_name': 'RiskName',
...
def transform_programme_risks(project_details: pd.DataFrame,
                              programme_risks: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    merged_df = pd.merge(project_details,
                         project_risks_subset,
                         on=['RiskName', 'RiskCategory', 'Short Description', 'Full Description', 'Consequences',
                             'Pre-mitigatedImpact', 'Pre-mitigatedLikelihood', 'Mitigatons', 'PostMitigatedImpact',
                             'PostMitigatedLikelihood', 'Proximity'],
                         how='outer')
...
def transform_project_funding_comments(project_details: pd.DataFrame,
    ...
    :return: Transformed DataFrame after merging and null row filtering.
    """
    with_project_id = get_project_id(project_funding_comments,
                                     lookup,
    ...
    merged_df = pd.merge(project_details,
                         project_funding_comments_subset,
                         on=['Project ID', 'Comment'],
                         how='outer')
...
def extract_project_funding_profiles(df_funding_profiles: pd.DataFrame) -> pd.DataFrame:
    ...
    df_funding_profiles['unsecured_funding'] = df_funding_profiles['unsecured_funding'].replace({'No': 'Yes', 'Yes': 'No'})
    
    # Create a subset with only the relevant columns
    df_funding_profiles = df_funding_profiles[['Project ID', 'funding_source_name', 'funding_source', 'unsecured_funding', 'time_period']]
    
    # Rename columns to correspond to data model names for easier merge
    df_funding_profiles = df_funding_profiles.rename(columns={'funding_source_name': 'Funding Source Name',
...
def transform_project_funding_profiles(project_details: pd.DataFrame,
    ...
    :return: Transformed DataFrame after merging and null row filtering.
    """
    with_project_id = get_project_id(project_funding_profiles,
                                     lookup,
                                     PLACE_NAME_ABBREVIATIONS,
                                     SUPPLEMENTARY_ABBREVIATION_MAPPINGS)
...
def extract_project_location(project_location: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # Create a subset with only the relevant columns
    project_location = project_location[['Project ID', 'place_name', 'primary_intervention_theme', 'site_location_type']]
    
    # Rename columns to correspond to data model names for easier merge
    project_location = project_location.rename(columns={'primary_intervention_theme': 'Primary Intervention Theme',
...
def transform_project_location(project_details: pd.DataFrame,
    ...
    :return: Transformed DataFrame after merging and null row filtering.
    """
    with_project_id = get_project_id(project_location,
                                     lookup,
                                     PLACE_NAME_ABBREVIATIONS,
                                     SUPPLEMENTARY_ABBREVIATION_MAPPINGS)
...
def extract_project_progress(df_progress: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # Create a subset with only the relevant columns
    df_progress = df_progress[['Project ID', 'start_date', 'completion_date', 'status', 'delivery_rag', 'spend_rag', 'risk_rag', 'commentary']]
    
    # Rename columns to correspond to data model names for easier merge
    df_progress = df_progress.rename(columns={'start_date': 'Start Date', 'completion_date': 'Completion Date', 'status': 'Project Delivery Status', 'delivery_rag': 'Delivery (RAG)', 'spend_rag': 'Spend (RAG)', 'risk_rag': 'Risk (RAG)', 'commentary': 'Commentary on Status and RAG Ratings'})
    
    return df_progress


def transform_project_progress(project_details: pd.DataFrame,
    ...
    :return: Transformed DataFrame after merging and null row filtering.
    """
    with_project_id = get_project_id(project_progress,
                                     lookup,
                                     PLACE_NAME_ABBREVIATIONS,
                                     SUPPLEMENTARY_ABBREVIATION_MAPPINGS)
    ...
    merged_df = pd.merge(project_details,
                         ...
                         on=['Project ID', 'Start Date', 'Completion Date', 'Project Delivery Status', 'Delivery (RAG)', 'Spend (RAG)', 'Risk (RAG)', 'Commentary on Status and RAG Ratings'],
                         how='outer')
...
def extract_project_risks(df_project_risks: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # Create a subset with only the relevant columns
    df_project_risks = df_project_risks[['Project ID', 'risk_name', 'risk_category', 'short_description_of_the_risk', 'full_description', 'consequences', 'pre-mitigated_impact', 'pre-mitigated_likelihood', 'mitigations', 'post-mitigated_impact', 'post-mitigated_likelihood', 'proximity']]
    
    # Rename columns to correspond to data model names for easier merge
    df_project_risks = df_project_risks.rename(columns={'risk_name': 'RiskName', 'risk_category': 'RiskCategory', 'short_description_of_the_risk': 'Short Description', 'full_description': 'Full Description', 'consequences': 'Consequences', 'pre-mitigated_impact': 'Pre-mitigatedImpact', 'pre-mitigated_likelihood': 'Pre-mitigatedLikelihood', 'mitigations': 'Mitigatons', 'post-mitigated_impact': 'PostMitigatedImpact', 'post-mitigated_likelihood': 'PostMitigatedLikelihood', 'proximity': 'Proximity'})
    
    return df_project_risks


def transform_project_risks(project_details: pd.DataFrame,
    ...
    :return: Transformed DataFrame after merging and null row filtering.
    """
    with_project_id = get_project_id(project_risks,
                                     lookup,
                                     PLACE_NAME_ABBREVIATIONS,
                                     SUPPLEMENTARY_ABBREVIATION_MAPPINGS)
    ...
    merged_df = pd.merge(project_details,
                         project_risks_subset,
                         on=['Project ID', 'RiskName', 'RiskCategory', 'Short Description', 'Full Description', 'Consequences', 'Pre-mitigatedImpact', 'Pre-mitigatedLikelihood', 'Mitigatons', 'PostMitigatedImpact', 'PostMitigatedLikelihood', 'Proximity'],
                         how='outer')
...
def get_project_id(round_1_category: pd.DataFrame,
                   lookup_table: pd.DataFrame,
                   abbreviations: dict,
                   supplementary_dict: dict) -> pd.DataFrame:
    """
    ...
    round_1_category['Project ID'] = prefix + '-' + round_1_category['place_name'].map(get_abbreviation) + '-' + round_1_category['proj_num'].astype(str).map(get_actual_proj_num)

    return round_1_category


def get_project_number(round_1_category: pd.DataFrame,
    ...
    if 'place_name' and 'proj_name' in round_1_category.columns:
        ...
        merged_df = pd.merge(round_1_category,
                             lookup_table,
                             on=['place_name', 'project_name'],
                             how='left')
    else:
        merged_df = None
